...
class Base(object):
    ...
    def read_data(self, bag):
        ...
        def posMap(pos):
            if pos < -self.p.max_pos:
                return 0
            elif pos > self.p.max_pos:
                return (self.p.max_pos + 1) * 2
            else:
                return pos + (self.p.max_pos + 1)
        ...
        for sent_ in bag['sents']:
            ...
            for dep in dep_edges:
                ...
                dep_links.append(
                    (dep['governor'] - 1, dep['dependent'] - 1, 0, 1))  # -1, because indexing starts from 0
            ...
            for edge in dep_links:
                ...
                if edge[1] == sub_pos or edge[1] == obj_pos:
                    if min(sub_pos, obj_pos) < edge[0] < max(sub_pos, obj_pos):
                        ...
                        right_nbd_phrase.add(tok_list[edge[0]])
    ...
    def predict(self, sess, data, wLabels=True, shuffle=False, split="valid", no_eval=False):
        ...
        for step, batch in enumerate(self.getBatches(split=split, shuffle=shuffle, no_eval=no_eval)):
            ...
            if step % 10 == 0:
                if no_eval:
                    self.logger.info(
                        'Evaluating {} ({}/{}):\t{}'.format(split, bag_cnt, self.total_bags_data, self.p.name))
                else:
                    self.logger.info(
                        'Evaluating {} ({}/{}):\t{:.5}\t{:.5}\t{}'.format(split, bag_cnt, self.total_bags_data,
        ...
        if not no_eval:
            return np.mean(losses), results, np.mean(
                accuracies) * 100, y, y_pred, logit_list, y_actual_hot, wrd_attens, e_pair
        else:
    ...
    def run_epoch(self, sess, epoch, shuffle=True):
        ...
        for step, batch in enumerate(self.getBatches(shuffle=shuffle)):
            ...
            if step % 10 == 0:
                self.logger.info(
                    'E:{} Train Accuracy ({}/{}):\t{:.5}\t{:.5}\t{}\t{:.5}'.format(epoch, bag_cnt, self.total_bags_data,
                                                                                   np.mean(accuracies) * 100,
                                                                                   np.mean(losses), self.p.name,
                                                                                   self.best_val_area))
