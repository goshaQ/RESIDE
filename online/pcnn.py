...
class PCNN(Base):
    ...
    def add_placeholders(self):
        ...
        self.part_pos = tf.placeholder(tf.int32, shape=[None, 2],
                                       name='input_part_pos')  # Subject and object position in the sentence.
    ...
    def create_feed_dict(self, batch, wLabels=True, dtype='train'):  # Where putting dropout for train?
        if not self.p.no_eval:
            X, Y, pos1, pos2, sent_num, part_pos = batch['X'], batch['Y'], batch['Pos1'], batch['Pos2'], batch[
                'sent_num'], batch['PartPos']
        else:
            X, pos1, pos2, sent_num, part_pos = batch['X'], batch['Pos1'], batch['Pos2'], batch['sent_num'], batch[
                'PartPos']
    ...
    def add_model(self):
        ...
        with tf.variable_scope('Convolution') as scope:
            ...
            kernel = tf.get_variable('kernel', [self.p.ngram, conv_in_dim, 1, self.p.num_filters],
                                     initializer=tf.truncated_normal_initializer(), regularizer=self.regularizer)
            biases = tf.get_variable('biases', [self.p.num_filters], initializer=tf.random_normal_initializer(),
                                     regularizer=self.regularizer)
        ...
        piece_pool = tf.reduce_max(convRes, axis=1)

        with tf.variable_scope('Sentence_attention') as scope:
            sent_atten_q = tf.get_variable('sent_atten_q', [self.p.num_filters, 1])

            def getSentAtten(num):
                bag_sents = piece_pool[num[0]: num[1]]
                num_sents = num[1] - num[0]
                sent_atten_wts = tf.nn.softmax(tf.reshape(tf.matmul(tf.tanh(bag_sents), sent_atten_q), [-1]))
                bag_rep_ = tf.reshape(tf.matmul(tf.reshape(sent_atten_wts, [1, -1]), bag_sents), [self.p.num_filters])
                return bag_rep_

            bag_rep = tf.map_fn(getSentAtten, self.sent_num, dtype=tf.float32)

        with tf.variable_scope('FC1') as scope:
            w_rel = tf.get_variable('w_rel', [self.p.num_filters, self.num_class],
                                    initializer=tf.truncated_normal_initializer(), regularizer=self.regularizer)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-nRelAlias', dest="RelAlias", action='store_true',
                        help='Restore from the previous best saved model')
    parser.add_argument('-n_de_gcn', dest="de_gcn", action='store_false',
                        help='Restore from the previous best saved model')
    ...
    parser.add_argument('-logdir', dest="log_dir", default='/scratchd/home/shikhar/entity_linker/src/cesi/log/',
                        help='Log directory')
    parser.add_argument('-config', dest="config_dir", default='/scratchd/home/shikhar/entity_linker/src/config/',
                        help='Config directory')
    parser.add_argument('-rel2id_map', default='../data/inell_relations.json',
                        help='File containing relation to id mapping')
    parser.add_argument('-rel2alias_file', default='../data/extended_m_inell_relations.json',
                        help='File containing relation to alias mapping')
    parser.add_argument('-type2id_file', default='../data/type2id.json', help='File containing type to id mapping')
    parser.add_argument('-merge_rel',
                        default='/scratchd/home/shikhar/entity_linker/data/riedel_data/merge_decisions.json',
                        help='File containing remapping of relations')
    ...
    with tf.Session(config=config) as sess:
        ...
        if not args.onlyTest:
            ...
            if not args.no_eval:
                test_loss, test_pred, test_acc, y, y_pred, logit_list, y_hot, wrd_attens, e_pair = model.predict(sess,
                                                                                                                 None,
                                                                                                                 split="test")
                ...
                logger.info(
                    'Main result (test): Prec:{} | Rec:{} | F1:{} | Area:{}'.format(test_prec, test_rec, test_f1,
                                                                                    area_pr))
                pickle.dump({'logit_list': logit_list, 'y_hot': y_hot, 'e_pair': e_pair},
                            open("results/{}/onlyTest_precision_recall.pkl".format(args.name),
                                 'wb'))  # Store predictions
            else:
                test_pred, y_pred, logit_list, wrd_attens, e_pair = model.predict(sess, None, split="test",
                                                                                  no_eval=True)
                pickle.dump({'logit_list': logit_list, 'e_pair': e_pair},
                            open("results/{}/onlyTest_precision_recall.pkl".format(args.name),
                                 'wb'))  # Store predictions
