    ...
    def add_model(self):
        ...
        with tf.variable_scope('Bi-LSTM') as scope:
            fw_cell = tf.contrib.rnn.DropoutWrapper(
                tf.nn.rnn_cell.GRUCell(self.p.lstm_dim, reuse=tf.AUTO_REUSE, name='FW_GRU'),
                output_keep_prob=self.rec_dropout)
            bk_cell = tf.contrib.rnn.DropoutWrapper(
                tf.nn.rnn_cell.GRUCell(self.p.lstm_dim, reuse=tf.AUTO_REUSE, name='BW_GRU'),
                output_keep_prob=self.rec_dropout)
        ...
        with tf.variable_scope('Word_attention', reuse=tf.AUTO_REUSE) as scope:
            wrd_query = tf.get_variable('wrd_query', [lstm_out_dim, 1],
        ...
        with tf.variable_scope('Sentence_attention', reuse=tf.AUTO_REUSE) as scope:
            sent_atten_q = tf.get_variable('sent_atten_q', [lstm_out_dim, 1],
                                           initializer=tf.contrib.layers.xavier_initializer())
