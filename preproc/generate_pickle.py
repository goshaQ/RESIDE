        ...
        for k, line in enumerate(f):
            ...
            for sent in bag['sents']:
                ...
                if len(bag['sub']) > len(bag['obj']):
                    ...
                    sub_start_off = [len(' '.join(sent['rsent'].split()[0: idx])) + (1 if idx != 0 else 0) for idx in
                                     sub_idx]
                    if sub_start_off == []: sub_start_off = [m.start() for m in
                    ...
                    obj_start_off = [len(' '.join(sent['rsent'].split()[0: idx])) + (1 if idx != 0 else 0) for idx in
                                     obj_idx]
                    if obj_start_off == []: obj_start_off = [m.start() for m in
                ...
                for s_n, corenlp_sent in enumerate(sent['corenlp']['sentences']):
                    ...
                    for dep in dep_edges:
                        ...
                        dep_links.append((tid_map[s_n][dep['governor']] - 1, tid_map[s_n][dep['dependent']] - 1, 0,
                                          1))  # -1, because indexing starts from 0
...
for dtype in ['train', 'test']:
    for i in range(len(data[dtype]) - 1, -1, -1):
        ...
        for j in range(len(bag['wrds_list']) - 1, -1, -1):
            ...
            data[dtype][i]['dep_links_list'][j] = [e for e in data[dtype][i]['dep_links_list'][j] if
